...
# 화면 배경 이미지 로드
background = pygame.image.load("assets/images/background_city1.png")  # 배경 이미지 파일을 로드합니다.
...
# 퀘스트 아이템 초기 설정
quests = [
    Quest("programming", item_colors[0], 
          screen_width, random.randint(0, screen_height - item_height), 
          item_speed),
    Quest("math", item_colors[1],
           screen_width + random.randint(200, 400), 
           random.randint(0, screen_height - item_height),
             item_speed),
    Quest("mini_game", item_colors[2],
...
def check_obstacle_collision(char_x, char_y, obstacle):
    global char_health
    if obstacle.active and (char_x < obstacle.x + item_width) and (char_x + char_width > obstacle.x) and (char_y < obstacle.y + item_height) and (char_y + char_height > obstacle.y):
...
# 장애물 초기 설정
obstacles = [
    Obstacle(screen_width + random.randint(50, 300), random.randint(0, screen_height - item_height), item_speed)
]


            
def start_screen():
    screen.fill((0, 0, 0))  # 검은 배경으로 초기화
